...
public class EquipPane extends ScrollPane {
    ...
    private Pane getDetailPane() {
        ...
        if (eqiuppedWeapon != null) {
            weaponLbl = new Label("Weapon:\n" + eqiuppedWeapon.getName());
            weaponImg.setImage(new Image(Launcher.class.getResource(eqiuppedWeapon.getImagepath()).toString()));
        ...
        // ข้อ 4
        Button Unequips = new Button();
        Unequips.setText("Unequips");
        Unequips.setOnAction(event -> Launcher.setUneqiupItem());
        ...
        equipmentInfoPane.getChildren().addAll(weaponLbl, weaponImgGroup, armorLbl, armorImgGroup, Unequips);
        return equipmentInfoPane;
    }

    public void drawPane(Weapon eqiuppedWeapon, Armor equippedArmor) {
        this.eqiuppedWeapon = eqiuppedWeapon;
